...
Функция ТекстТекущейТаблицы(ОписаниеТаблицы, ПолноеИмяМетаданного, ОсновнаяТаблица, ПараметрыЗапроса)
	...
	Иначе
		ДобавитьПолеУсловий(ПоляТаблицы, ПоляУсловий, "Ссылка", Объект, ПараметрыЗапроса);
	КонецЕсли;
...
Функция ПравоеЗначениеСУБД_MSSQL(ПравоеЗначение)
	...
	Иначе
		ТекстСсылки = ПолучитьНавигационнуюСсылку(Объект);
		ИДСсылкиВБазе = Прав(ТекстСсылки, 32);
