...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы Тогда
		ЗаполнитьСписокИзбраннымиСтроками();
		СохранитьНастройки();
	КонецЕсли;
...
&НаКлиенте
Процедура ОтобратьПоПодсистемам(Команда)

	// BSLLS:UnusedLocalVariable-off
	ИндексАктивнойСтроки = 0;
	// BSLLS:UnusedLocalVariable-off

	ОписаниеОповещения = Новый ОписаниеОповещения("Подключаемый_ОбработатьСписокПодсистем", ЭтотОбъект);
	// BSLLS:UnusedLocalVariable-off
	ОткрытьФормуРедактора("ОтборОбъектовПоПодсистемам", Неопределено, ЭтотОбъект, , ОписаниеОповещения);
	// BSLLS:UnusedLocalVariable-off

КонецПроцедуры

&НаКлиенте
Процедура УстановитьПризнакИзбранного(Команда)

	ТекДанные = Элементы.ОбъектыМетаданных.ТекущиеДанные;
	РодительТекДанные = ТекДанные.ПолучитьРодителя();
	Если ТекДанные = Неопределено Или РодительТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;

	ТекДанные.Избранное = Не ТекДанные.Избранное;
	РодительТекДанные.Избранное = ТекДанные.Избранное;

	УстановитьКартинкуКнопкиФормы(ТекДанные.Избранное, Элементы.ОбъектыМетаданныхУстановитьПризнакИзбранного);

	Если РодительТекДанные.Избранное Тогда
		Возврат;
	КонецЕсли;

	ПодчиненныеЭлементыРодителя = РодительТекДанные.ПолучитьЭлементы();
	Для Каждого ПодчиненныйЭлементРодителя Из ПодчиненныеЭлементыРодителя Цикл
		Если ПодчиненныйЭлементРодителя.Избранное Тогда
			РодительТекДанные.Избранное = ПодчиненныйЭлементРодителя.Избранное;
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ОтобразитьИзбранное(Команда)

	УстановленныйПризнакИзбранного = Не УстановленныйПризнакИзбранного;
	УстановитьКартинкуКнопкиФормы(УстановленныйПризнакИзбранного, Элементы.ОбъектыМетаданныхОтобразитьИзбранное);
...
&НаКлиенте
Процедура УстановитьКартинкуКнопкиФормы(Избранное, КнопкаФормы)

	КнопкаФормы.Картинка = ?(Избранное, БиблиотекаКартинок.ДобавитьВИзбранное, БиблиотекаКартинок.Избранное);
	КнопкаФормы.Заголовок = ?(Избранное, НСтр("ru = 'Удалить из избранного'"), НСтр("ru = 'Добавить в избранное'"));
...
Процедура ЗаполнитьОбъектыМетаданных()
	...
	Для Каждого ТекущаяГруппа Из РеквизитФормыВЗначение("Объект").ГруппыМетаданных() Цикл
		...
		УзелГруппы.КартинкаМетаданного = БиблиотекаКартинок[ТекущаяГруппа.ПрефиксЗапроса];
		УзелГруппы.Видимость = Истина;
		УзелГруппы.Избранное = УстановитьЗначениеИзбранного(УзелГруппы.Имя);
		...
		Для Каждого ОбъектМетаданных Из Метаданные[ТекущаяГруппа.Имя] Цикл
			...
			УзелОбъекта.КартинкаМетаданного = БиблиотекаКартинок[ТекущаяГруппа.ПрефиксЗапроса];
			УзелОбъекта.Видимость = Истина;
			УзелОбъекта.Избранное = УстановитьЗначениеИзбранного(УзелОбъекта.Имя);
...
&НаСервере
Функция УстановитьЗначениеИзбранного(Имя)

	Возврат Не СписокИзбранныхСтрок.НайтиПоЗначению(Имя) = Неопределено;
...
Процедура Подключаемый_ОбработатьВыборМетаданного() Экспорт
	...
	Если ТекущиеДанные <> Неопределено И ТекущиеДанные.ПолучитьРодителя() <> Неопределено Тогда
		...
		Элементы.ОбъектыМетаданныхУстановитьПризнакИзбранного.Видимость = Истина;
		УстановитьКартинкуКнопкиФормы(ТекущиеДанные.Избранное, Элементы.ОбъектыМетаданныхУстановитьПризнакИзбранного);

	Иначе
		ПрефиксЗапроса = "";
		ИмяОбъекта = "";
		ГруппаМетаданных = "";
		Элементы.ОбъектыМетаданныхУстановитьПризнакИзбранного.Видимость = Ложь;
		УстановитьКартинкуКнопкиФормы(Ложь, Элементы.ОбъектыМетаданныхУстановитьПризнакИзбранного);
	КонецЕсли;
	// BSLLS:UnusedLocalVariable-on
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	ЭлементыОбъектовМетаданных = Элементы.ОбъектыМетаданных.ПодчиненныеЭлементы;

	УстановкаВидимости(ЭлементыОбъектовМетаданных);
	УстановкаИзбранного(ЭлементыОбъектовМетаданных);

КонецПроцедуры

&НаСервере
Процедура УстановкаВидимости(ЭлементыОбъектовМетаданных)

	НовыйЭлементУО = УсловноеОформление.Элементы.Добавить();
	НовыйЭлементУО.Использование = Истина;

	УсловиеОтбора(НовыйЭлементУО, "ОбъектыМетаданных.Видимость", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	УсловиеОтбора(НовыйЭлементУО, "УстановленныйПризнакВидимость", ВидСравненияКомпоновкиДанных.Равно, Истина);

	УстановитьОформлениеПолейЭлементаУО(НовыйЭлементУО, ЭлементыОбъектовМетаданных);

	НовыйЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);

КонецПроцедуры

&НаСервере
Процедура УстановкаИзбранного(ЭлементыОбъектовМетаданных)

	НовыйЭлементУО = УсловноеОформление.Элементы.Добавить();
	НовыйЭлементУО.Использование = Истина;

	УсловиеОтбора(НовыйЭлементУО, "ОбъектыМетаданных.Избранное", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	УсловиеОтбора(НовыйЭлементУО, "УстановленныйПризнакИзбранного", ВидСравненияКомпоновкиДанных.Равно, Истина);
...
&НаСервере
Процедура УстановитьОформлениеПолейЭлементаУО(НовыйЭлементУО, ПодчиненныеЭлементы)

	Для Каждого ПодчиненныйЭлемент Из ПодчиненныеЭлементы Цикл
		Если ТипЗнч(ПодчиненныйЭлемент) = Тип("ГруппаФормы") Тогда
			УстановитьОформлениеПолейЭлементаУО(НовыйЭлементУО, ПодчиненныйЭлемент.ПодчиненныеЭлементы);
			Продолжить;
		КонецЕсли;

		УстановитьОформлениеПолей(НовыйЭлементУО, ПодчиненныйЭлемент.Имя);
	КонецЦикла;
...
Процедура УстановитьУсловноеОформлениеСписка()
	...
	УсловиеОтбора(НовыйЭлементУО, "Ссылка", ВидСравненияКомпоновкиДанных.НеРавно, "ЭтоКонстанта");

	УстановитьОформлениеПолей(НовыйЭлементУО, "Ссылка");
...
&НаСервере
Процедура УсловиеОтбора(НовыйЭлементУО, ЛевоеЗначение, ВидСравнения, ПравоеЗначение)
...
&НаСервере
Процедура УстановитьОформлениеПолей(НовыйЭлементУО, Поле)
...
Процедура ВосстановитьНастройки()
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		...
		Если Настройки.Свойство("ИзбранныеСтроки") Тогда
			// BSLLS:UnusedLocalVariable-off
			СписокИзбранныхСтрок = Настройки.ИзбранныеСтроки;
			// BSLLS:UnusedLocalVariable-on
		КонецЕсли;
...
Процедура СохранитьНастройки()
	...
	Настройки.Вставить("ИзбранныеСтроки", СписокИзбранныхСтрок);
...
&НаСервере
Процедура ЗаполнитьСписокИзбраннымиСтроками()

	СписокИзбранныхСтрок.Очистить();

	КорневыеУзлы = ОбъектыМетаданных.ПолучитьЭлементы();
	Для Каждого КорневойУзел Из КорневыеУзлы Цикл
		КорневойУзелУстановлен = Ложь;
		ПодчиненныеУзлы = КорневойУзел.ПолучитьЭлементы();
		Для Каждого ПодчиненныйУзел Из ПодчиненныеУзлы Цикл
			Если Не ПодчиненныйУзел.Избранное Тогда
				Продолжить;
			КонецЕсли;

			СписокИзбранныхСтрок.Добавить(ПодчиненныйУзел.Имя);
			Если Не КорневойУзелУстановлен Тогда
				КорневойУзелУстановлен = Не КорневойУзелУстановлен;
				СписокИзбранныхСтрок.Добавить(КорневойУзел.Имя);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
&НаКлиенте
Процедура Подключаемый_ОбработатьСписокПодсистем(РезультатЗакрытия, ДополнительныеПараметры) Экспорт

	Если ТипЗнч(РезультатЗакрытия) = Тип("Неопределено") Тогда
		Возврат;
	КонецЕсли;

	УстановленныйПризнакВидимость = ЗначениеЗаполнено(РезультатЗакрытия);

	УстановитьВидимостьПолейДерева(ОбъектыМетаданных.ПолучитьЭлементы(), "Видимость", Не УстановленныйПризнакВидимость);

	ОтборСтрокПоСоставуПодсистем(РезультатЗакрытия);

	Элементы.ОбъектыМетаданных.ТекущаяСтрока = 
		ОбъектыМетаданных.ПолучитьЭлементы().Получить(ИндексАктивнойСтроки).ПолучитьИдентификатор();

КонецПроцедуры

&НаКлиенте
Процедура УстановитьВидимостьПолейДерева(ЭлементыДерева, ПолеСтроки, ЗначениеПоляСтроки)

	Для Каждого ЭлементДерева Из ЭлементыДерева Цикл
		ЭлементДерева[ПолеСтроки] = ЗначениеПоляСтроки;
		УстановитьВидимостьПолейДерева(ЭлементДерева.ПолучитьЭлементы(), ПолеСтроки, ЗначениеПоляСтроки);
	КонецЦикла;

КонецПроцедуры

&НаСервере
Процедура ОтборСтрокПоСоставуПодсистем(Знач Подсистемы)

	ОтсутствуютДанные = Не Подсистемы.Количество();
	...
	Для Каждого КорневойУзел Из КорневыеУзлы Цикл
		ПодчиненныеУзлы = КорневойУзел.ПолучитьЭлементы();
		Для Каждого ПодчиненныйУзел Из ПодчиненныеУзлы Цикл
			ВхождениеОбъектаВСостав(КорневыеУзлы, КорневойУзел, ПодчиненныйУзел, Подсистемы, ОтсутствуютДанные);
		КонецЦикла;
	КонецЦикла;
...
&НаСервере
Процедура ВхождениеОбъектаВСостав(КорневыеУзлы, КорневойУзел, ПодчиненныйУзел, Подсистемы, ОтсутствуютДанные)

	Для Каждого Подсистема Из Подсистемы Цикл
		СоставПодсистемы = Метаданные.НайтиПоПолномуИмени(Подсистема).Состав;
		СодержитОбъект = СоставПодсистемы.Содержит(Метаданные[ПодчиненныйУзел.ГруппаМетаданных][ПодчиненныйУзел.Имя]);

		Если СодержитОбъект Тогда
			ПодчиненныйУзел.Видимость = СодержитОбъект;
			КорневойУзел.Видимость = ПодчиненныйУзел.Видимость;
			Если Не ОтсутствуютДанные Тогда
				// BSLLS:UnusedLocalVariable-off
				ИндексАктивнойСтроки = КорневыеУзлы.Индекс(КорневойУзел);
				// BSLLS:UnusedLocalVariable-on
				ОтсутствуютДанные = Не ОтсутствуютДанные;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
