...
Процедура ТекстЗапроса(Команда)
	...
	ПараметрыФормы.Вставить("Объект", ТекущийОбъект);
			...
			Для Каждого ТабличнаяЧасть Из ТабличныеЧасти Цикл
				...
				Для Каждого СтрокаТаблицы Из ЭтотОбъект[ТабличнаяЧасть.Имя] Цикл
					...
					Для Каждого ТекущийРеквизит Из ТабличнаяЧасть.Реквизиты Цикл
						...
						Если Прав(ТекущийРеквизит.Значение, 23) = "_ПредставлениеХранилища" Тогда
							Продолжить;

						ИначеЕсли Прав(ТекущийРеквизит.Значение, ДлинаСуффикса) = "_АдресХранилища" Тогда
...
&НаСервере
Процедура УстановитьВидимостьДоступность(СоздаватьЭлементыФормы = Ложь)
...
&НаСервере
Процедура ЗаполнитьДанныеОбъекта(СоздаватьЭлементыФормы = Истина)
	...
	ИначеЕсли ГруппаМетаданных = "РегистрыСведений" Тогда
		...
		ЗаполнитьРеквизитыОбъекта(МенеджерЗаписи, ОбъектМетаданных.СтандартныеРеквизиты, 14,
			СтруктураХраненияБазы, ПолноеИмяМетаданного);
		ЗаполнитьРеквизитыОбъекта(МенеджерЗаписи, ОбъектМетаданных.Измерения, 17,
			СтруктураХраненияБазы, ПолноеИмяМетаданного);
		ЗаполнитьРеквизитыОбъекта(МенеджерЗаписи, ОбъектМетаданных.Ресурсы, 18,
			СтруктураХраненияБазы, ПолноеИмяМетаданного);
		ЗаполнитьРеквизитыОбъекта(МенеджерЗаписи, ОбъектМетаданных.Реквизиты, 15,
			СтруктураХраненияБазы, ПолноеИмяМетаданного);
	...
	Иначе
		...
		СтрокаСвойства = СвойстваОбъекта.Добавить();
		СтрокаСвойства.ИндексКартинки = 16;
		...
		ЗаполнитьРеквизитыОбъекта(ОбъектИсточник, ОбъектМетаданных.СтандартныеРеквизиты, 14,
			СтруктураХраненияБазы, ПолноеИмяМетаданного);
		ЗаполнитьРеквизитыОбъекта(ОбъектИсточник, ОбъектМетаданных.Реквизиты, 15,
			СтруктураХраненияБазы, ПолноеИмяМетаданного, ЭтоГруппа);
...
Процедура ИзменитьРегистрациюНаУзле(Узел, Объект, Зарегистрировать, ГруппаОбъекта)
...
&НаСервереБезКонтекста
Процедура ВыбратьИзмененияНаУзле(Узел, Объект, НомерСообщения)
...
Процедура УдалитьНепосредственноНаСервере()
	...
	ТекущийОбъект.ПолучитьОбъект().Удалить();
...
Процедура ПровестиДокументНаСервере()
	...
	ТекущийОбъект.ПолучитьОбъект().Записать(РежимЗаписиДокумента.Проведение);
...
Процедура ОтменитьПроведениеДокументаНаСервере()
	...
	ТекущийОбъект.ПолучитьОбъект().Записать(РежимЗаписиДокумента.ОтменаПроведения);
...
&НаСервереБезКонтекста
Функция ПометкаУдаленияУстановлена(Ссылка)
...
&НаСервере
Процедура ИзменитьПометкуУдаленияНаСервере(ЗначениеПометки)
	...
	ТекущийОбъект.ПолучитьОбъект().УстановитьПометкуУдаления(ЗначениеПометки);
